...
const ticketData = {
  ...
  jamSampai: "08.00 WIB",
  kelas: [
    { nama: "Ekonomi (Dewasa) x1", harga: "Rp78.000", tempat: "E.01" },
    { nama: "Bisnis (Dewasa) x1", harga: "Rp91.000", tempat: "B.01" },
  ],
      ...
      <Card className="w-fit py-0 gap-0">
        ...
        <CardContent className="p-4 space-y-4">
          ...
          <div className="flex justify-between">
            ...
            <div className="text-end md:mr-4">
              <div className="mb-2">
                <p>Asal</p>
        ...
        <CardFooter className={cn("md:flex-row items-start flex-col border-t-2 border-dashed px-0")}>
          ...
          <div className="p-4 w-full  space-y-4 md:border-l-2 border-dashed md:p-4">
            ...
            <div className="md:flex md:gap-x-13 space-y-2 md:space-y-0">
              <div>
                <p>Kelas</p>
                {ticketData.kelas.map((data, index) => (
                  <p key={index} className="font-semibold">{data.nama}</p>
                ))}
              </div>
              <div>
                <p>Harga</p>
                {ticketData.kelas.map((data, index) => (
                  <p key={index} className="font-semibold">{data.harga}</p>
                ))}
              </div>
              <div>
                <p>Tempat</p>
                {ticketData.kelas.map((data, index) => (
                  <p key={index} className="font-semibold">{data.tempat}</p>
                ))}
              </div>
            </div>
            <div className="flex justify-between md:gap-8">
              ...
              <div>
                <p className="md:text-white">Harga</p>
